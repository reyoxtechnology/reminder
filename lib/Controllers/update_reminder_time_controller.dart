...
class UpdateReminderTimeController extends GetxController{
  var url = Uri.parse("https://reminder.bitcash.ng/api/profile/setTime");

  setReminderTime(String selectedTime) async {
    ...
    await HttpClient().putMethod(headers, body, url).then((value){
      if(value.statusCode == 200 || value.statusCode == 201){
        ...
        Get.to(()=>DashBoard());
        alertBar(Get.context!, "Reminder Time Updated", AppTheme.secondary.withOpacity(0.3), false, Icon(Icons.done));
      }else{
        CustomProgressDialog().popCustomProgressDialogDialog(Get.context!);
